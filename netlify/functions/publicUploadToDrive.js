...
exports.handler = async (event) => {
    // This is a public function for clan logo uploads. It does NOT require a JWT.
    // Its security comes from the fact that it can only upload to one specific, designated folder.
    ...
    try {
        ...
        // The frontend sends the image as a base64 encoded string. We need to convert it back to a buffer.
        const buffer = Buffer.from(file.split(',')[1], 'base64');
        const bufferStream = new stream.PassThrough();
        bufferStream.end(buffer);

        // Define the metadata for the file we are about to create on Google Drive.
        const fileMetadata = {
            // Add a timestamp to the file name to prevent naming conflicts if two clans upload "logo.png".
            name: `clan-logo-${Date.now()}-${fileName}`, 
            parents: [GOOGLE_DRIVE_FOLDER_ID], // This ensures the file goes into our designated "Crimson Banners" folder.
        };
        
        // Define the media content of the upload request.
        const media = {
            mimeType: 'image/jpeg', // We can assume jpeg/png for logos.
            body: bufferStream,
        };
        ...
        // Google Drive requires a specific URL format to display images directly.
